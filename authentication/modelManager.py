...
def EmployeeSerializer(Email, Username, Password, Dob, CITIZENSHIP_NO, Address, PhoneNo, EmployeeType):
    account = AccountDB.objects.create(email=Email, username=Username, password=Password, DateOfBirth=Dob,
                                       CITIZENSHIP_NO=CITIZENSHIP_NO, Address=Address, Phone_No=PhoneNo)
    account.set_password(Password)
    account.save()
    return EmployeeDB.objects.create(Account_ID=account, Employee_Type=EmployeeType)


def MemberSerializer(Email, Username, Password, Dob, CITIZENSHIP_NO, Address, PhoneNo):
...
def GetMemberDetails(memberId):
    ...
    try:
        ...
        CITIZENSHIP_NO = AccountDB.objects.filter(id=accountId).values('CITIZENSHIP_NO')
        ...
        dict['CITIZENSHIP_NO'] = CITIZENSHIP_NO[0]['CITIZENSHIP_NO']
